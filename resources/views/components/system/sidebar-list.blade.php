...
<li class="nav-item {{ $subMenuIsActive === true ? 'menu-open' : '' }}">
    <a href="{{ isset($input['hasSubmenu']) && $input['hasSubmenu'] ? '#' : route($input['route'] . '.index') }}"
        class="nav-link {{ $subMenuIsActive === true ? 'active' : '' }} {{ isset($input['permission']) && $input['permission'] == false ? 'd-none' : '' }} {{ !isset($input['hasSubmenu']) && request()->segment(2) === $input['route'] ? 'active' : '' }}
